...
| The speed | The in-flight HTTP request aborts at once. A running tool always finishes. |
...
The user's request to stop one turn. One object, shared by the interface and the loop.

| Member | Visibility | Definition |
|---|---|---|
| `request()` | public | The user asked to stop. Sets the flag, and interrupts the armed thread. |
| `requested()` | public | The loop asks whether to stop. |
...
disarms would land on the tool that runs next. The lock makes `disarm` wait for the interrupt,
and then clear it.
...
2. It arms the cancellation around `client.chat` only, and disarms in a `finally`. The interrupt
   therefore aborts the HTTP request through the `InterruptedException` path that `OpenAiClient`
   already has. A tool is never interrupted.
3. The `catch (LlmException)` asks `cancellation.requested()` first. An aborted request is a stop,
   not a transport failure.
4. It asks `requested()` before each tool call, and after each iteration.
...
`LlmClient`, `Tool`, `ToolPolicy`, `ToolResult`, `Conversation` and `Ui` do not change.
...
The reply carries two tool calls. The user presses ESC while the first one runs.
...
User         Watcher    Cancellation      Agent     Conversation      Tool
  ...
  │             │─request()──►│             │             │             ┃  runs on
  │             │             │             │             │             ┃
  │             │        flag := true, and no thread is armed,          ┃
...
**The tool always finishes.** No thread is armed while a tool runs. `request` sets the flag and
does nothing else. `EditFile` cannot stop between the write and the move.

**The result is still appended.** The loop asks `requested()` before each tool call, not after.
The first call therefore completes the normal way, and the check that stops the turn happens where
the second call would have started.
...
| Class | Change |
|---|---|
| `Main` | Builds one `Cancellation`. Passes it to `selectUi` and to the `Agent`. |
...
| `AgentTest` | ESC during the provider call returns `Stopped.`, and the history ends with the user message and `AssistantMessage("Stopped by the user.")`. ESC during tool 1 of 2: tool 1 completes, tool 2 never executes, and both calls carry a `ToolMessage`. After `respond` returns, `Thread.interrupted()` is false. |
...
- `CLAUDE.md`: rows for `Cancellation` and `EscapeWatcher`, and the `Agent` row.
